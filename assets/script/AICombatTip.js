            ...
            function (e) {
                ...
                if (gameMap[cur_i][cur_j] != 0) {
                    ...
                    if (gameMap[cur_i][cur_j].substr(2, 1) == 'j') {
                        ...
                        // 发送请求
                        wx.request({
                            url: 'http://'+localHost+'/chess/aihouse/succ', //仅为示例，并非真实的接口地址
                            data: {
                ...
                wx.request({
                    ...
                    url: 'http://'+localHost+'/chess/aihouse/move', //仅为示例，并非真实的接口地址
                    timeout:10000,
                    data: {
                      protocol:timeStamp+'<>'+'red'+'<>'+curName+','+cur_i+','+cur_j
                    //   chessName: curName,
                    //     pre_i: pre_i,
                    //     pre_j: pre_j,
                    //     cur_i: cur_i,
                    ...
                    success: function (res) {
                      ...
                      if (res.statusCode == 200) {
                        ...
                        if(res.data.code==1){
                            ...
                            pre_i = pre_y > 0 ? (pre_y - 30) / 60 + 5 : (pre_y + 30) / 60 + 4;//先前逻辑坐标
                            pre_j = pre_x / 68 + 4;
                            // cc.find("Canvas/game_bg/grid/"+host+"_box"+"/"+host+"_box"+(pre_i*9+pre_j+1)).active = true;
                            cur_i = parseInt(res.data.x);
                            cur_j = parseInt(res.data.y);
                            // curName = 'b_m_l';
                            // pre_i = 9;
                            // pre_j = 1;
                            // cur_i = 7;
                            ...
                            des_x = (cur_j - 4) * 68;
                            des_y = (cur_i - 4) > 0 ? (cur_i - 5) * 60 + 30 : (cur_i - 4) * 60 - 30;
                            // pre_node = cc.find("Canvas/game_bg/grid/chess/" + curName);
                            if (gameMap[cur_i][cur_j] != 0) {
                                cc.find("Canvas/game_bg/grid/chess/" + gameMap[cur_i][cur_j]).active = false;
                                enemyName = gameMap[cur_i][cur_j];
